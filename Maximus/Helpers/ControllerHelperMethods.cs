...
namespace Maximus.Helpers
{
    public class ControllerHelperMethods
    {
        ...
        ////    result = entity.getcustassemblies.Where(x => x.ParentStyleID == style & x.isChargeable == false & x.CustID == busId).Select(x => new AssemblyModel { StyleID = x.StyleID, Instruction = x.Instruction, IsChargeable =Convert.ToInt32(x.isChargeable) }).ToList();
        ////    result1 = entity.getallassemblies.Any(x => x.ParentStyleID == style & x.isChargeable == false) ? entity.getallassemblies.Where(x => x.ParentStyleID == style & x.isChargeable == false).Select(x => new AssemblyModel { StyleID = x.StyleID, Instruction = x.Instruction, IsChargeable = Convert.ToInt32(x.isChargeable) }).ToList() : new List<AssemblyModel>();
        ////    result.AddRange(result1);
        ////    if (result.Count == 0)
        ////    {
        ////        result = entity.getallassemblies.Where(x => x.ParentStyleID == style & x.isChargeable == false).Select(x => new AssemblyModel { StyleID = x.StyleID, Instruction = x.Instruction, IsChargeable = Convert.ToInt32(x.isChargeable) }).ToList();
        ////    }
        ////    long curLine = lineNo;
        ...
        ////}
        ////public List<SalesOrderLineViewModel> GetOptionalAssembly(List<string> assemList = null, string style = "", long lineNo = 0,List<SalesOrderLineViewModel> lines=null, long qty = 0, string empId = "", string empName = "", int lastLino = 0, string busId = "")
        ////{
